...
# 创建 MultiheadAttention 对象
attn = MultiheadAttention(embed_dim, num_heads)

# 生成随机数据
query = torch.rand(query_len, batch_size, embed_dim)
key = torch.rand(key_len, batch_size, embed_dim)
value = torch.rand(key_len, batch_size, embed_dim)

# 调用 MultiheadAttention
output, weights = attn(query, key, value, attn_mask=None)
