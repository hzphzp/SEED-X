# Tongyi Qianwen is licensed under the Tongyi Qianwen 
...
from src.models.mllm.utils import load_zero3_checkpoint
...
class Resampler(nn.Module):
    ...
    def __init__(self, grid_size, embed_dim, num_heads, kv_dim=None, norm_layer=nn.LayerNorm):
        ...
        self.pos_embed = nn.Parameter(torch.from_numpy(get_2d_sincos_pos_embed(embed_dim,
                                                                               grid_size)).float()).requires_grad_(False)
...
class VisualAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        context_layer = context_layer.view(b, self.num_attention_heads_per_partition, sq, self.hidden_size_per_attention_head)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(
        ...
        self.resblocks = nn.ModuleList(
            [VisualAttentionBlock(width, heads, mlp_ratio, act_layer=act_layer, norm_layer=norm_layer) for _ in range(layers)])
    ...
    def forward(self, x: torch.Tensor, attn_mask: Optional[torch.Tensor] = None):
        ...
        for r in self.resblocks:
            if gradient_checkpointing and self.training:
                x = torch.utils.checkpoint.checkpoint(r, x, None, None, attn_mask)
            else:
...
class VisionTransformerWithAttnPool(nn.Module):
    ...
    def __init__(self,
        ...
        if patch_pos:
            ...
            self.patch_pos_embed = nn.Parameter((output_dim**-0.5) * torch.randn(4, output_dim))
    ...
    def forward(self, x: torch.Tensor, patch_positions: Optional[torch.Tensor] = None):
        ...
        if self.patch_pos:
            ...
            rel_posembed = torch.mm(torch.cat([patch_positions, 1 - patch_positions], dim=-1) / 2, self.patch_pos_embed).unsqueeze(1)
            x = x + rel_posembed
        x = self.ln_post(x)
        x = x @ self.proj
    ...
    def from_pretrained(cls, pretrained_model_path=None, **kawrgs):
        if os.environ.get('DEBUG_FLAG', 'False') == 'True':
            print('DEBUG_FLAG is set to True, return a random initialized model')
            kawrgs.update(
                {
                    "heads": 4,
                    "image_size": 448,
